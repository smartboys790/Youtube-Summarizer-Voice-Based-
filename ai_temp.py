...
def chat2(data):
    try:
        ...
        # Split data into chunks of 3000 characters
        data_chunks = split_query_into_chunks(data)
        ...
        for chunk in data_chunks:
            ...
            if response.status_code == 200:
                ...
                response_data = response.json()
                assistant_reply = response_data.get('choices')[0].get('message').get('content')
                combined_context += assistant_reply  # Append the current chunk's response
            else:
                return f"Error: {response.status_code}"
...
def ask_question(query):
    ...
    for chunk in query_chunks:
        ...
        payload = json.dumps({
            ...
            "messages": [
                {"role": "system", "content": system_message},
                {"role": "user", "content": combined_context + chunk}
            ],
            "repetition_penalty": 1.1,
            "temperature": 0.7,
            "top_p": 0.9,
...
data = "yeh ek bada data hai jo ki chunks me bhejna hai"
questions = ["yeh ek question hai jo ki chunks me bhejna hai", "yeh dusra question hai", "yeh teesra question hai"]

ask_question_func = chat2(data)  # Call chat2 to get the ask_question function
